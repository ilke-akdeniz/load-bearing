...
One thing has to be separated out first, or chapter 04 refutes this chapter in a sentence.
...
Chapter 04 argues for the first, for a reason with nothing to do with substitution: a component reaching for `os.Getenv("DATABASE_URL")` is holding decisions that were never its to make. That argument stands and this chapter does not touch it.
...
`GetForUpdate` is there because somewhere a balance is read and then written, and chapter 05 owns why that needs the row held still. Against Postgres it is one clause:
...
Chapter 08's rate layers put the schema below the code and moving more slowly. The interface lives in the fast layer. What has to move on migration day — rows, types, constraints, indexes, the queries a planner was tuned for, the operational runbook — lives in the slow one.
...
- Cutting over per-tenant or per-route rather than all at once — chapter 11's strangler fig.
...
That is chapter 02's reversibility rule doing its work: this is cheap to do at migration time and expensive to do speculatively, so deferring it is a plan rather than a bet.
...
An interface is a means. Stability is the test — which is chapter 04's reading of this same sentence, arrived at there from the mechanism and confirmed here by the source.
...
That is a scope statement written down by the author in the original paper. Chapter 14's case was a scope given aloud once, in a talk, and lost because nobody re-watched it. This one has been in print since 1994.
...
The Force is chapter 02's *control of the callers* pointed at the substrate instead — you do not control the environment your code runs in. Everything above assumes you do, and that there is one production database whose name you chose.
...
- **A second implementation that exists only in tests.** That is chapter 16's subject, and it means the insurance framing was never the real reason.
...
- **"We'll swap it out later if we need to."** *Later* is the tell. An interface with a reason that survives deleting that word is one chapter 04 would defend.
...
**Next:** Part V turns from diagnosis to method — chapter 18 sets out how to read the Forces in front of you, derive the Principles they support, and check the Idioms of the language you are writing in, in that order.
