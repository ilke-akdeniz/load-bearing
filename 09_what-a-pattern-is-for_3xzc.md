...
What happens next is the problem, and it is a mechanism the book has already named. [Chapter 03](03_grading-a-law_q5c6.md) draws the line between a claim that **describes** what happens and one that **prescribes** what to do — and notes that only a prescription can be bad advice, because only a prescription is advice. A catalogue describes. Read as a checklist, "shapes that occur" silently becomes "shapes you should have," and a list of observations becomes a list of obligations.
...
The check is the one [chapter 02](02_forces_f4m5.md) gives for any deferred decision: write down what would have to become true for the name to be settled. *This is `pending/` until the third importer lands, and then we split it* is a different artifact from `pending/` with no note attached, even though the code is identical.
