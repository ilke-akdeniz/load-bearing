...
A Law has a mechanical consequence: violate its preconditions and the program is wrong, and you can go and be wrong (Ch. 04). A Principle has a Force with a value, and the value can be looked up — the row count, the number of writers (Ch. 03). An Idiom has a compiler or a runtime that acts on the choice, so there is a machine you can ask (Ch. 02), and chapter 21 works out what that machine's answer costs.
...
### The question that was never a Style question
...
This is the general shape of the boundary and it is worth stating plainly: **before treating something as Style, check that both options produce the same program.** If they do not, the thing is not Style and the discussion has a fact in it after all — which means it can be settled, and settling it is worth the time.

Chapter 21 gives the systematic version of the same trap: the line between Style and Idiom is drawn in a different place by each language, so a decision that is arbitrary in one is structural in another.
...
**Next:** chapter 23 closes the book by putting the five kinds to work on the thing you actually meet — a blog post, a review comment, a colleague's strong opinion, and code a machine wrote that states no decisions at all.
