...
**The two are also not independent, and the connection is this book's rather than the paper's.** [claude connetion is not mine or this books, back then, every single book I read about TDD or tutorials would talk about that "connection". And it's not really a connection. Fast unit tests, red green refactor is part of the TDD culture or approach, or framework, whatever you call it. ] What is taught is not an ordering but a loop, run at minutes: write a failing test, make it pass, refactor, repeat. The median cycle in the study was about eight minutes, and short cycles are the part the evidence credits. A suite you run every eight minutes has to answer in seconds, and a suite that answers in seconds does not start a database — *the tests must be fast* being the standard justification for replacing one with a double.
