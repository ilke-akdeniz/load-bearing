...
**A word on *scope*, because the book has used a different one for this.** Earlier chapters say a Principle has **conditions** — what must be true for it to hold, which is always some fact about your Forces. Chapter 04's information-hiding Principle has a sharp one: *you do not control your callers.*
...
*Gofmt's style is no one's favorite, yet gofmt is everyone's favorite* is on the same list of nineteen, and it became a norm — mandatory automatic formatting, no configuration — without incident. There is no situation in which one consistent format is wrong, so a reader who receives only the proverb has received all of it. Chapter 13 reached this boundary from the other side: a compressed statement is safe exactly when its condition is *always*. Chapter 21 is the general case.
...
- **"That's just how it's done here."** Sometimes a real Idiom with reasons behind it (Ch. 20), sometimes a wide reading nobody has revisited. One question separates them.
...
**Next:** chapter 15 is the first of the three cases — object orientation's advice about where behaviour belongs, and the references that appear in both directions once two rules are placed by it.
