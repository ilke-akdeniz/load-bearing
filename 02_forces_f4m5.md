...
[Chapter 05](05_time_mdbn.md) owns the races themselves; [chapter 06](06_distribution_49yh.md) owns why redelivery cannot be eliminated and idempotency is the answer.
...
That is not evidence of a careless team. It is a structure whose boundaries do not line up with the way change actually arrives, so every change crosses all of them. [Chapter 04](04_structure_agjy.md) owns why fan-in sets the price of a change; [chapter 17](17_abstraction-as-insurance_4jk6.md) owns what these particular boundaries cost.
...
The second version is not free: it costs a package boundary and a constructor call at every site, and it is worth that only at some sizes. [Chapter 11](11_patterns-that-survive-translation_us2k.md) catalogues the technique; the point here is that **the same rule needs a different mechanism at a different team size**, and neither mechanism is the better engineering in general.

**AI coding agents sit at the extreme of both halves.** The Force asks how many people must agree and how many will still be here, and for coding agents the answer to the second is *nobody*. The model was present for no conversation, keeps nothing between sessions, and arrives at a volume the review step was not sized for. The Forces that would settle the decisions in the model are facts about your situation, held by you. And the agent can only reach it to the extent that some prompt happened to carry them.
...
**What changes with the Force:** not the design, the *plan*. This is the Force that decides how much a mistake costs to correct, which is why it belongs in the room before the API is designed rather than after. [Chapter 04](04_structure_agjy.md) owns what this implies for what you expose in the first place.
...
- **A read-modify-write in a request handler.** A concurrency Force present and unread, and the symptom is silent data loss: two requests read the same value, both write, and the second overwrites the first with no error anywhere ([Ch. 05](05_time_mdbn.md)).
- **Retry logic with no idempotency key.** Half of a Force read: the failure was anticipated, the redelivery it causes was not ([Ch. 06](06_distribution_49yh.md)).
