...
**Listed patterns** — the rest of each family, one line each. These are not explained, only placed. By now you have [chapter 09](09_what-a-pattern-is-for_3xzc.md)'s tests, so a one-line entry is something you can evaluate rather than something you have to accept.

Each worked pattern carries two labelled lines, and they answer different questions.

- ***The constraint*** — what the pattern **forbids** once you adopt it. This is [chapter 09](09_what-a-pattern-is-for_3xzc.md)'s second test applied: a name that rules nothing out carries no information, so a pattern with no constraint is not a pattern. It is not a list of prerequisites; it is what you may no longer do.
...
[Chapter 05](05_time_mdbn.md) owns the Law here and [chapter 06](06_distribution_49yh.md) owns what happens across machines. These are the shapes that answer them.
...
*The constraint:* two parts of one operation cannot hold divergent copies of the same row, which is a lost-update race ([Ch. 05](05_time_mdbn.md)) that no amount of care at the call sites removes.
...
- **Optimistic offline lock** — a version column; the writer whose version is stale is refused. [Chapter 05](05_time_mdbn.md) works it through.
...
- **Single writer** — remove the contention rather than manage it. [Chapter 05](05_time_mdbn.md) owns this, and [chapter 07](07_scale_637f.md) shows what it does to throughput.
- **Idempotency key** — [chapter 06](06_distribution_49yh.md) owns it; it is what makes at-least-once delivery survivable.
- **Saga** — [chapter 06](06_distribution_49yh.md) owns it; the answer when the unit of consistency spans systems and no transaction can.
...
- **Transactional Outbox** — [chapter 06](06_distribution_49yh.md) owns this one, and it is what you reach for when Unit of Work's constraint cannot be met.
...
- **Circuit breaker** — stop calling a failing dependency, so its slowness stops consuming your capacity. [Chapter 09](09_what-a-pattern-is-for_3xzc.md) uses the name as its example of a term that is a mediocre description and an excellent search key.
- **Timeout with backoff and jitter** — bound the wait, and spread the retries so recovery does not arrive as a synchronized stampede ([Ch. 06](06_distribution_49yh.md)).
...
Every pattern here is a seam placed where two things move at different speeds — [chapter 08](08_change_rjf9.md)'s rate-of-change layers, made structural.
...
*The cost:* an interface per boundary, and the reflex to add one everywhere is how a codebase acquires forty interfaces with one implementation each ([Ch. 16](16_tdd-and-mocks_u8eu.md)).
...
- **Pipes and filters** — stages that transform and hand on, each replaceable without the others. [Chapter 04](04_structure_agjy.md) notes these are not layers, and the difference matters.
...
- **Repository** — a collection-like interface over storage. Worth [chapter 09](09_what-a-pattern-is-for_3xzc.md)'s tests before adopting: it compresses well, and what it rules out is thinner than its reputation suggests.
...
- **Anti-corruption layer** — [chapter 10](10_patterns-that-cross_r8dw.md) owns it: what a translation boundary becomes when the thing on the other side is not yours to change.
...
- **Composition root** — one place where the object graph is assembled, so a newcomer reads one file rather than tracing a graph ([Ch. 04](04_structure_agjy.md)).
...
[Chapter 07](07_scale_637f.md) supplies the arithmetic underneath all of these.
...
[Chapter 08](08_change_rjf9.md)'s compatibility rule and [chapter 10](10_patterns-that-cross_r8dw.md)'s ownership line both land here. The patterns are ways of making a boundary survivable.
...
Adding a field is the one change [chapter 08](08_change_rjf9.md) says is always safe, so a reader that fails on it has turned their safe change into your outage. The tolerant version simply does not look:
...
*The cost:* a field that disappears reads as its zero value rather than as an error, which is the silent failure of [chapter 08](08_change_rjf9.md) and the price of the tolerance.
...
*The cost:* both sides must adopt the tooling, and a broker is one more piece of infrastructure to run. More seriously, **a consumer who does not participate is invisible** — the green build says "no recorded expectation broke," not "nobody broke." The pattern is exactly as good as its coverage, and it converts [chapter 04](04_structure_agjy.md)'s unknowable dependency set into a known-but-incomplete one.

**The rest of this family**

- **Bounded context** — one model per context, with translation between them, rather than one model everyone must agree on. [Chapter 08](08_change_rjf9.md)'s Conway material is why the boundaries end up where they do.
- **Composition root** — one place where the object graph is assembled, so nothing else needs to know how anything is built ([Ch. 04](04_structure_agjy.md)).
- **Expand and contract** — add the new field, migrate readers, then remove the old one, in three deploys rather than one. [Chapter 08](08_change_rjf9.md)'s add-only rule is what forces the shape.
...
**They last because a Force outlives a language.** Concurrency was a problem in 1970 and is a problem now. Data outlives code in COBOL and in Rust. Someone else always depends on your interface. A pattern answering one of those describes the shape of the problem rather than a gap in a toolchain — which is why it is still recognizable after being carried into a language its author never used. [Chapter 12](12_missing-language-features_esqm.md) takes the converse: a name that disappears when the language changes was answering the language, not the problem.
...
**Some answer what the problem is rather than what the situation is.** A state machine is the right shape when the domain genuinely has states and transitions — an order that is placed, then paid, then shipped. That is a fact about the business, not about your concurrency or your latency budget. The same goes for Transaction Script, which [chapter 09](09_what-a-pattern-is-for_3xzc.md) uses as its compression example: it is what you write when *no* Force is pushing you anywhere else, and it is right far more often than its reputation suggests.
...
Run [chapter 09](09_what-a-pattern-is-for_3xzc.md)'s tests before using any of them, and [chapter 10](10_patterns-that-cross_r8dw.md)'s question before believing the cost estimate. A list is a map of what exists, which is a different thing from a set of instructions.
...
- **"That's the standard pattern for this."** Standard where, and answering which Force? [Chapter 10](10_patterns-that-cross_r8dw.md)'s question applies to the recommendation too.
