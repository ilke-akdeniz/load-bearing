...
- `00_toc.md` — the contents page: twenty-five chapters in five parts, each a number, a title and a link, and nothing else.
...
[Chapters 17](17_behaviour-placement_z47a.md), [18](18_tdd-and-mocks_u8eu.md) and [19](19_abstraction-as-insurance_4jk6.md) are case studies. [Chapter 16](16_principle-loses-scope_b86v.md) makes the claim and keeps the general rubric; they are three instances of it, and they do not each get a claim of their own.
...
6. **What it costs** — where both sides have a bill, two sections: *what following the advice costs* and *what taking the alternative costs*. Mixing them in one list is what made [chapter 18](18_tdd-and-mocks_u8eu.md)'s costs section unreadable.
...
[Chapter 16](16_principle-loses-scope_b86v.md) got the Pike material from a third-party transcript, read in excerpts, and then failed in a way excerpt-reading makes almost inevitable.
...
[Chapter 18](18_tdd-and-mocks_u8eu.md) shipped a draft with three stacked block quotes, one of them containing *"this advice would require a negative (statistically significant) coefficient, which the models did not produce."*
...
The TOC once carried a prose summary of every chapter, and those summaries described their chapters wrongly whenever a chapter changed during review — [chapter 14](14_missing-language-features_esqm.md)'s said *"Decorator is function composition"* after the chapter had measured that and found it false.
...
**Both a chapter and a review are worked through by interview, not by delivering a result and waiting for objections.** [Chapter 20](20_force-map-method_r37x.md) documents the technique and quotes the prompt it comes from; this is the same thing, run by Claude, with the author answering.
...
- **Order the questions by dependency.** Ask the root decision first. [Chapter 21](21_six-profiles_dnkz.md)'s terminology question came first because the title, every heading, the TOC entry, three ledger rows and two cross-references all inherited from it, and asking anything else first would have meant asking it twice.
...
- **Keep a running note as the answers land.** Four questions in, the early exchanges are easy to reconstruct wrongly and easy to reconstruct confidently. The author's exact words are worth more than the paraphrase: [chapter 22](22_idioms_7nkn.md)'s two counter-arguments were sharper than the summary written from memory an hour later would have been.

**When it is not worth it.** The technique is slow by design, which [chapter 20](20_force-map-method_r37x.md) states as one of its costs. A review that is three typos and a wording fix is applied, not interviewed. The test is whether any item would change what the other items should be — where nothing depends on anything, there is no tree to walk, and an interview is ceremony.
...
Three times while finishing [chapter 16](16_principle-loses-scope_b86v.md), a single word caught in review was doing the same wrong job elsewhere, and the one-site fix would have left the rest.
...
[Chapter 21](21_six-profiles_dnkz.md)'s rename shipped alongside its edits and came out at forty-three, so `git show` reported a deleted file and a new one and the review diff was unreadable at exactly the moment somebody wanted to read it.
