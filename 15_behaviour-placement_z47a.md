...
This is the first of Part IV's three cases. Chapter 14 made the claim; these three are what it looks like happening, and this one is the case where the author of the advice saw the compression coming and wrote against it in his introduction.
...
`Order` now holds a `Customer`. One edge, pointing one way. Nothing is wrong yet, and chapter 04's Law is not yet engaged: a single edge is a dependency, not a cycle.
...
Chapter 04 lists four ways to remove a cycle. This is one of them: replace the reference with an identifier. The price is a lookup — somewhere there must now be code that takes a `CustomerID` and fetches the customer.
...
Where a rule needs one entity, the advice is right as stated, and this chapter's caution produces the failure chapter 13 describes: the rule is moved away from the only data it reads, for no gain.
...
**Every traversal becomes explicit work.** With references, `customer.orders` is free at the call site. With identifiers, somebody must fetch the orders for a customer, and the code that does it has to exist, be tested, and be called in the right order. Chapter 11's N+1 problem is what happens when this is done carelessly.

**The rule's location stops being obvious.** On an entity, a rule has one plausible home and everybody finds it. At a service that sees both entities, there are several plausible homes, and without a stated method for choosing — which chapter 18 owns — the placement becomes a matter of local habit.

**Flat structures read as procedural, and that carries a social cost.** A team fluent in object modelling will recognize the shape as the one chapter 13 defends against a verdict noun, and the argument has to be made again on every review. The technical answer is settled and the vocabulary is not.
...
- **"It's an anemic domain model"** used against flat structures with identifiers. Chapter 13 has the answer: the term carries an antecedent about wasted mapping cost that the speaker has to establish before the word means anything.
...
**Next:** chapter 16 turns to testing, where *test behaviour, not implementation* meets the mock that asserts about itself.
