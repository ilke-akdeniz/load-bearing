...
So this chapter sorts the field against chapter 02's seven Forces, in chapter 02's order. Forty-nine patterns fall into them, and a handful answer the shape of the problem instead — a state machine is right when the domain has states, which is a fact about the business rather than about your circumstances. What is left over after both is the interesting residue, and it is dealt with in the boundary section.
...
**Listed patterns** — the rest of each family, one line each. These are not explained, only placed. By now you have chapter 09's tests, so a one-line entry is something you can evaluate rather than something you have to accept.

Each worked pattern carries two labelled lines, and they answer different questions.

- ***The constraint*** — what the pattern **forbids** once you adopt it. This is chapter 09's second test applied: a name that rules nothing out carries no information, so a pattern with no constraint is not a pattern. It is not a list of prerequisites; it is what you may no longer do.
...
Chapter 05 owns the Law here and chapter 06 owns what happens across machines. These are the shapes that answer them.
...
*The constraint:* two parts of one operation cannot hold divergent copies of the same row, which is a lost-update race (Ch. 05) that no amount of care at the call sites removes.
...
- **Optimistic offline lock** — a version column; the writer whose version is stale is refused. Chapter 05 works it through.
...
- **Single writer** — remove the contention rather than manage it. Chapter 05 owns this, and chapter 07 shows what it does to throughput.
- **Idempotency key** — chapter 06 owns it; it is what makes at-least-once delivery survivable.
- **Saga** — chapter 06 owns it; the answer when the unit of consistency spans systems and no transaction can.
...
- **Transactional Outbox** — chapter 06 owns this one, and it is what you reach for when Unit of Work's constraint cannot be met.
...
- **Circuit breaker** — stop calling a failing dependency, so its slowness stops consuming your capacity. Chapter 09 uses the name as its example of a term that is a mediocre description and an excellent search key.
- **Timeout with backoff and jitter** — bound the wait, and spread the retries so recovery does not arrive as a synchronized stampede (Ch. 06).
...
Every pattern here is a seam placed where two things move at different speeds — chapter 08's rate-of-change layers, made structural.
...
*The constraint:* the interface is declared by the consumer rather than the provider, which is what reverses the arrow — a provider-declared interface leaves the dependency pointing exactly where it was (Ch. 04).

*The cost:* an interface per boundary, and the reflex to add one everywhere is how a codebase acquires forty interfaces with one implementation each (Ch. 16).
...
- **Pipes and filters** — stages that transform and hand on, each replaceable without the others. Chapter 04 notes these are not layers, and the difference matters.
...
- **Repository** — a collection-like interface over storage. Worth chapter 09's tests before adopting: it compresses well, and what it rules out is thinner than its reputation suggests.
...
- **Anti-corruption layer** — chapter 10 owns it: what a translation boundary becomes when the thing on the other side is not yours to change.
...
- **Composition root** — one place where the object graph is assembled, so a newcomer reads one file rather than tracing a graph (Ch. 04).
...
Chapter 07 supplies the arithmetic underneath all of these.
...
*The constraint:* the cached copy may be stale, so the design is not the lookup — it is the invalidation. A copy with no invalidation strategy is a copy that is allowed to be wrong (Ch. 03).
...
- **Object pool** — reuse expensive-to-create things. Chapter 07's warning applies: measure, because a pool can easily cost more than the allocation it avoids.
...
Chapter 08's compatibility rule and chapter 10's ownership line both land here. The patterns are ways of making a boundary survivable.
...
Adding a field is the one change chapter 08 says is always safe, so a reader that fails on it has turned their safe change into your outage. The tolerant version simply does not look:
...
*The cost:* a field that disappears reads as its zero value rather than as an error, which is the silent failure of chapter 08 and the price of the tolerance.
...
*The cost:* both sides must adopt the tooling, and a broker is one more piece of infrastructure to run. More seriously, **a consumer who does not participate is invisible** — the green build says "no recorded expectation broke," not "nobody broke." The pattern is exactly as good as its coverage, and it converts chapter 04's unknowable dependency set into a known-but-incomplete one.

**The rest of this family**

- **Bounded context** — one model per context, with translation between them, rather than one model everyone must agree on. Chapter 08's Conway material is why the boundaries end up where they do.
- **Composition root** — one place where the object graph is assembled, so nothing else needs to know how anything is built (Ch. 04).
- **Expand and contract** — add the new field, migrate readers, then remove the old one, in three deploys rather than one. Chapter 08's add-only rule is what forces the shape.
...
**They last because a Force outlives a language.** Concurrency was a problem in 1970 and is a problem now. Data outlives code in COBOL and in Rust. Someone else always depends on your interface. A pattern answering one of those describes the shape of the problem rather than a gap in a toolchain — which is why it is still recognizable after being carried into a language its author never used. Chapter 12 takes the converse: a name that disappears when the language changes was answering the language, not the problem.
...
That is a real gap in this chapter's method, not a defect in the patterns. Chapter 16 covers the testing material, and it is organized by what the techniques actually buy rather than by Force, for exactly this reason.

**Some answer what the problem is rather than what the situation is.** A state machine is the right shape when the domain genuinely has states and transitions — an order that is placed, then paid, then shipped. That is a fact about the business, not about your concurrency or your latency budget. The same goes for Transaction Script, which chapter 09 uses as its compression example: it is what you write when *no* Force is pushing you anywhere else, and it is right far more often than its reputation suggests.
...
Knowing the Force narrows the field; it rarely closes it. *Writers collide* gives you optimistic locking, pessimistic locking, single-writer partitioning, and a serializable transaction, and choosing between them needs the Force's **intensity** — chapter 02's dial — plus what you are willing to pay.

This chapter sorts. It does not decide. Chapter 18 is the one that turns a set of Forces into a design.
...
Run chapter 09's tests before using any of them, and chapter 10's question before believing the cost estimate. A list is a map of what exists, which is a different thing from a set of instructions.
...
**Naming the Force is genuinely hard, and this chapter makes it look easy.** In a real design the Forces arrive tangled, and half of them are estimates about the future (Ch. 02). The neat question at the head of each section is the output of the analysis, not the input.

**A grouped list still invites shopping.** Chapter 09's warning about catalogues applies here too: six well-organized shelves are still shelves, and *we should use a Saga* is as available a sentence after reading this as before. The defence is the same — say which Force, and at what intensity.
...
- **"That's the standard pattern for this."** Standard where, and answering which Force? Chapter 10's question applies to the recommendation too.
- **"We'll need CQRS eventually."** Eventually is chapter 02's territory: does the decision expire, and is it cheap today? For CQRS, both answers are unfavourable.
...
**Next:** chapter 12 takes the converse of this chapter's test. If a pattern survives translation because it answers a Force, then a pattern that *disappears* when you change language was answering the language — and a surprising share of the best-known catalogue turns out to be exactly that.
