...
None of this requires two speakers. A generator trained on text covering all five kinds returns all five in one register, and the usual correction — learning whose confidence to discount — has nobody to attach to. [-- let's remove previous two paragraphs, the late addition didn't mesh well]
