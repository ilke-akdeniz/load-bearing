...
And he is explicit that much of what people argue about is not the kind of thing that has a right answer — which is chapter 01's subject stated by someone with every reason to claim otherwise.

## Already used

**Chapter 14** takes the concurrency section: Ousterhout's *threads are bad*, whose situation was pthreads in a particular kind of program and which Google banned company-wide anyway; and Pike's own admission that the concurrency use cases were server software, that the team should have said so, and that the confusion between concurrency and parallelism probably drove some programmers away.
See decisions 72 and 73.

**Chapter 20** takes both pieces owed to it: interfaces colouring their designers' thinking, as the boundary on 21's claim, and the early compiler in C as the worked deviation.
...
One further line was noticed and left for chapter 01, which already owes it a paragraph: Pike lists *using upper case for export* alongside *where the newlines go* as perennial arguments, which is 02's point that tone does not vary with authority.

## Material owed to other chapters

### Chapter 01 — confidence does not track the kind of claim
...
Chapter 01's mechanism is that tone does not vary with authority, so a Style question and a Law arrive sounding alike.
...
### Chapter 08 — a compatibility promise, priced by the person who made it
...
Chapter 08 owns *once published, it is forever* and the add-only rule.
...
### Chapter 20 — an Idiom that limited the people who invented it
...
Chapter 20 takes Idioms seriously rather than dismissing them.
...
### Chapter 20 — a deviation from Idiom, defended by Forces
...
Chapter 20's contents already promise *when to deviate, and how to pay for it — declare it, document the reason, keep it narrow*.
...
### Chapter 12 — lower confidence, and worth checking before use
...
This is adjacent to chapter 12 rather than in it.
Chapter 12 is about patterns that dissolve when the language has the feature; this is about a feature that creates work the programmer must then carry, which is closer to an inverse.
Do not force it. If it fits anywhere it may be chapter 19's distributed-services section, or nowhere.

**2026-08-23: nowhere, on the evidence of chapter 19 being drafted.** Coloured functions is an observation about what a language feature costs its users, not about a domain's force profile, and 20's distributed section is almost entirely chapter 06's material cited. Forcing it in would have been a fifth thing in a section that adds one. Leaving it recorded here in case chapter 20 or 23 wants it.

### Chapter 04 — background rather than evidence
...
Chapter 04 owns dependency structure and does not need this to make its argument.
...
Material of this kind is strongest when the chapter is live, because the argument can be shaped around it — chapter 14's use of this source worked for that reason, and a quotation bolted onto a finished chapter is the decoration the register rules exclude.
