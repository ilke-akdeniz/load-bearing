...
| Prominence makes advice travel and its scope findable | 15 | Only the first gets used. Where advice came from a conversation there is nothing to go back to; where it came from someone with a book, the conditions are minutes away and the compressed version spreads regardless | "prominence cuts both ways (Ch. 15)" |
| The failure is retrieval, not preservation | 15 | In Part IV's four cases the conditions were published, often beside the advice, and the folk version travelled anyway — because the compressed form is complete enough to act on, and nobody interrupts an action they can already take | "retrieval, not preservation (Ch. 15)" |
...
| The instruction travels, the qualifiers do not | 16 | Riel's 2.9 means one key abstraction split in two, which excludes a rule over two entities; the reengineering pattern names that case and its cost, a dependency from provider back to client. The five words carry the instruction alone | "the qualifiers stayed behind (Ch. 16)" |
...
| *Database* is the word that slipped | 18 | Martin means the product; ordinary usage means the product plus the schema, the queries and the rows. Applying the ordinary meaning to his sentence yields *your schema is a mechanism*, which the same chapter denies — *the data is significant, the database is a detail* | "which database do you mean (Ch. 18)" |
| Deferral read as concealment | 18 | In the source a detail is a choice that can wait — *you don't have to decide upon them up front*; in the folk version it is something to hide, which is a wall built up front, the one move the source warns against | cite 18 |
...
| Plurality vs sequential replacement | 19 | Two implementations live at once is what an interface is for; replacing one engine with another forever is not, and only the first is a Force — see `docs/pending/speculative-abstraction.md` | "plurality, not replacement (Ch. 19)" |
| The criterion was dropped, not the scope | 19 | Martin's 1994 paper names stability as the test and an interface as one means; the five words that travel keep the technique and omit the test, because the technique is checkable in review and stability is a claim about the future | "the criterion, not the scope (Ch. 19)" |
| Stability comes from plurality | 19 | The source's reason an abstraction is stable is that many implementations depend on it — so a one-implementation interface fails the principle's own test while carrying its name | "stability needs dependents (Ch. 19)" |
| The caveat was printed and still lost | 19 | Martin's last paragraph says the standard may suit only certain applications and that he would regret unconditional conformance — 15's mechanism on advice whose scope was written down, not merely spoken | cite 19 |
| Insurance that cannot pay out | 19 | An abstraction bought against a future swap is shaped by the engine it was written against, sits in the code layer while the migration is a data problem, and costs the features you already run | "the premium is paid daily (Ch. 19)" |
| Injection is not abstraction | 19 | Passing a dependency in and hiding it behind an interface are two decisions; 05 argues for the first, and only the second is the speculative one | cite 19 |
| The interface publishes a capability | 19 | `GetForUpdate` is on the interface because Postgres has row locks; SQLite cannot implement it at all, so the abstraction promoted one engine's feature to a contract with its own callers | "the method names a capability (Ch. 19)" |
| The lowest common denominator is unknown | 19 | It is the intersection of feature sets for engines nobody has chosen, so it gets approximated by superstition — `for update` is absent from SQLite, `on conflict` is not | cite 19 |
| Premium continuous, payout singular | 19 | The cost is paid daily in small amounts by people who do not know they are paying; the payout is one future event that mostly does not occur, so experience never disconfirms the practice | "the cost and the payout arrive apart (Ch. 19)" |
| *Later* is the tell | 19 | An interface whose reason survives deleting the word *later* is one chapter 05 would defend | cite 19 |
| Force-map method | 20 | Read forces, derive principles, check idioms — in that order | cite 20 |
| An Idiom bounds its own designers | 22 | Pike: interfaces coloured the team's thinking for more than a decade, so every proposed polymorphism had to be reconciled with them — the cost is not thoughtless obedience, it is what the convention makes unimaginable | "the Idiom bounded them (Ch. 22)"; argument in `docs/pending/pike-retrospective.md` |
| A deviation, declared and dated | 22 | The early Go compiler in C: reason stated, offence taken, and the reason later expired and the deviation reversed | cite 22 |
| Domain inversions | 21 | Each domain's dominant force inverts some standard advice | cite 21 |
...
| Grilling | 20 | An interview that surfaces each decision, with a recommendation, before anything is written; the human supplies the Force that settles it | "grilling (Ch. 20)"; text, provenance and limits in `docs/pending/ai-material.md` |
| Grilling's limit | 20 | It surfaces decisions the model recognizes as decisions, so it is weakest where the corpus is most uniform | cite 20; argument in `docs/pending/ai-material.md` |
| Silent defaults | 24 | Generated code states no decisions at all — a taken branch leaves no mark, so review cannot catch what it never suspected | "silent default (Ch. 24)"; argument in `docs/pending/ai-material.md` |
...
| `NewOrders(*sql.DB)` against `NewOrders(Repository)` | 19 | Injection and abstraction as two separable decisions, with 05 arguing only for the first |
| `select … for update` refused by SQLite | 19 | `OperationalError: near "for": syntax error`, beside `on conflict` succeeding — the second implementation cannot satisfy the interface, and the lowest common denominator is not the obvious list |
