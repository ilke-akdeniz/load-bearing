...
| Why the kinds get confused | 01 | Tone doesn't vary with authority; advocacy compresses; monoculture makes Idioms feel universal | one clause, cite 01 |
| The Idiom/Style line is mechanical | 01 | The compiler or the runtime acts on an Idiom and neither can see a Style. What is held constant is narrow and named — what the program produces when it succeeds — so failure modes, setup effort and how much the compiler can check are all free to differ. Ecosystem-specificity does not discriminate, because plenty of Style is local. 20 owns the fact that the line falls in a different place in each language | "the machine can see it (Ch. 01)" |
| Law inert vs Principle wrong | 01 | A Force decides whether a Law *binds*; it decides whether a Principle is *right* | one clause, cite 01 |
...
| The seven Forces | 02 | Concurrency, durability of the medium, blast radius, change frequency and shape, team size and turnover, latency budget, control of callers | name the Force, cite 02 |
| Concurrency | 02 | How many at once, and do they touch the same state — it binds where writers collide, not where they coexist | cite 02; 05 owns the races |
| Durability of the medium | 02 | How long what this writes outlives the code that wrote it | cite 02; 08 owns published compatibility |
| Blast radius | 02 | When it is wrong, what happens and who finds out — decides how much prevention is worth | cite 02 |
| Change frequency and shape | 02 | How often, and how many places must change with it | cite 02; 04 owns fan-in pricing |
| Team size and turnover | 02 | How many must agree, and how many will still be here — the rule migrates comment → review → type system | cite 02 |
| Latency budget | 02 | What the budget is, and what fraction one mechanism costs | cite 02; 07 owns the arithmetic |
| Control of the callers | 02 | Can I change every call site, and would I know if I broke one — three values, not two | cite 02; 04 owns what it implies for exposure |
...
| Strict is the undoable direction | 02 | Under uncertainty prefer the decision you can walk back, which is usually the stricter one | cite 02 |
| Risk vs unmeasured Force | 02 | An unmeasured Force has an instrument and a trigger; a risk has neither, so the answer is bounding the cost rather than estimating harder | cite 02 |
| Shape of scale | 02 | "High scale" names no design — steady load, bursts, and data volume are different situations sharing a vocabulary | cite 02 |
| Forces move on their own clock | 02 | Team size, client count, and row count change without a commit, invalidating Principles nobody revisits | cite 02 |
...
| One kind, its moves | 03 | Theorem: falsify an assumption, or stop needing the conclusion. Definition: check whether the words apply. Empirical: measure it where you are | cite 03 |
| Two escapes from a theorem | 03 | Arrange for one of its assumptions not to hold, or stop needing the theorem's conclusion — never argue with the conclusion | cite 03 |
| Regularity vs magnitude | 03 | A measurement is not a law; an empirical law is a regularity across systems whose magnitude varies between them | cite 03 |
| Kind is not importance | 03 | The kind says how firmly a claim is true, not how much it bears on your program | cite 03 |
...
| Theorem and folk version share a name | 03 | The folk version has usually dropped a quantifier or a condition — halting problem forbids a universal decider, not any decision | cite 03 |
| The falsification question | 03 | "What would have to be true for this to be false?" — nothing / nothing but it may not apply / a measurement | cite 03 |
| Acyclic dependency | 04 | A cycle makes two components one unit of comprehension, test, and change | "the Direction Rule (Ch. 04)" |
| What a cycle actually costs | 04 | The damage is denominated in future change, not in wrong output — which is why it accumulates unnoticed | cite 04 |
| Injection vs inversion | 04 | Injection decides who constructs; inversion decides who declares the interface, and only inversion turns an arrow around | cite 04 |
| Two-phase construction | 04 | `a := &A{}; b := &B{a}; a.b = b` is the construction site admitting the cycle | cite 04 |
| Dependency damage compounds | 04 | A missing test is a static cost; a cycle spreads to everything touching either end | one clause, cite 04 |
| Cycle detection is granularity-bound | 04 | Each toolchain checks the boundaries it happens to have; the damage exists at all of them | cite 04 |
...
| Layering as ranks | 04 | Strict layering assigns every part a rank and forbids reaching past the one beneath it | one clause, cite 04 |
| Layer ≠ directory | 04 | A layer is a rule about call direction; a folder is neither necessary nor sufficient for it | cite 04; 17 owns what folders *cost*, and no chapter may say the physical forms enforce the same thing — 18's claim is that they do not |
| Cost of change scales with dependents | 04 | Change cost is proportional to fan-in, and is paid on every change | cite 04 |
| Stability, not indirection | 04 | "Depend on abstractions" means put what changes least at the bottom; an interface is not automatically that | cite 04 |
| Internal vs external dependent count | 04 | Inside the repo `grep` gives you the number; once published it is unknown and growing | cite 04 |
| From direction to surface | 04 | Cycles are about which way edges point; hiding is about how many edges exist at all | cite 04 |
| Information hiding / Hyrum | 04 | What is observable will be depended upon | cite 04 |
| DI does not contradict hiding | 04 | Injection is a module declining to hold decisions that belong elsewhere; the composition root holds them instead | cite 04 |
| Hiding's condition | 04 | Information hiding is a Principle conditional on not controlling your callers | cite 04; the Force itself is 03's |
| Export surface as liability | 04 | Every exported identifier is a contract; the surface is an inventory of what you can no longer change | cite 04 |
| A disclaimer is not a boundary | 04 | An "unsupported" export is still an export — Hyrum's Law does not read comments | cite 04 |
| Claim two, loose vs strict | 04 | "Dependencies flow downward" is claim one restated; the strict rank rule is what most systems fail | cite 04 |
| Graph vocabulary | 04 | Arrow = depends on; fan-in = who breaks if you change; bottom = high fan-in, low fan-out | cite 04 |
| The graph exists at every size | 04 | Functions through services; size changes what detects a violation, not whether the Law binds | cite 04 |
| Four ways to break a cycle | 04 | Interface / event / third module / identifier — four different bills, chosen deliberately | cite 04 |
| Dependency inversion | 04 | The call may go up while the dependency goes down, because both parties point at an interface at the bottom | cite 04 |
| Lower layer more capable | 04 | Layering doctrine assumes the layer below is dumber; when it is more capable, "keep logic out" inverts | cite 04 |
...
| Three conditions for a race | 05 | Another writer, a decision that depends on the read, and a rule spanning data you did not hold still — all three, or nothing to fix | cite 05 |
| The three ordinary fixes | 05 | One operation; let the data-holder enforce; or do not check and handle the failure | cite 05 |
| Lock the span, not the steps | 05 | The fix is not more locking but locking the right span | cite 05 |
| One authority beats many clocks | 05 | Optimistic concurrency on a version counter needs no clock; the database is the single source of order | cite 05 |
| Check-then-act / TOCTOU | 05 | Between the check and the act, the world moved | "TOCTOU (Ch. 05)" |
| Locking each step is not locking the sequence | 05 | Individually safe operations do not compose into a safe rule | cite 05 |
| The window is as wide as the work in it | 05 | The same defect is rare on an idle machine and constant under load | cite 05 |
| Only the lock-holder enforces | 05 | A rule over rows you haven't read can't be enforced by code that hasn't read them | cite 05 |
| The app check is the message, not the guarantee | 05 | Keep it for the error text; never keep it as the enforcement | cite 05 |
| Two ways to remove a race | 05 | Remove the sharing or remove the mutability; concurrency is the term you cannot give up | cite 05 |
...
| Clocks do not order events | 05 | Wall clocks lack the resolution locally and agreement globally; counters order, clocks do not | cite 05 |
| Lamport vs vector clocks | 05 | Lamport preserves causality but cannot detect concurrency; vector clocks can, at a cost that grows with nodes | cite 05 |
| Coordination does not compose | 05 | Two correct locked operations are not one correct operation | cite 05 |
...
| Exactly-once impossible | 06 | Two Generals ⇒ at-least-once plus idempotency | cite 06 |
| Every timeout is a guess | 06 | The observation is identical whether the peer is slow or dead, and the slow one may have committed | cite 06 |
| Idempotency key rules | 06 | The client generates it before the first attempt, and it commits in the same transaction as the effect | cite 06 |
| Two systems cannot share a transaction | 06 | No ordering of two commits is safe; the outbox makes the obligation to publish durable state, turning a permanent loss into a delay | cite 06 |
| Publish-then-delete, never the reverse | 06 | The drain loop chooses at-least-once over at-most-once, because duplicates are recoverable and a lost event is not | cite 06 |
| CAP, FLP, Two Generals by assumption | 06 | The proofs are not the point; the assumptions are the only negotiable part | cite 06 |
| PACELC over CAP | 06 | The else-branch — latency against consistency — applies every day; CAP's branch only during a partition | cite 06 |
...
| Three ways to stop multiplying | 06 | Remove the dependency, make it optional, or make it asynchronous | cite 06 |
| Saga is not rollback | 06 | Compensations are business operations and are visible to customers | cite 06 |
| Five shapes of scale | 07 | Ceiling, knee, superlinear curve, discontinuity, floor — the shape decides the fix | "which shape (Ch. 07)" |
| Amdahl: a ceiling | 07 | The serial fraction bounds speedup regardless of core count; shrink s rather than buying cores | cite 07 |
| USL: returns go negative | 07 | Contention grows with workers and coherency with pairs, so past a peak each added worker lowers throughput | cite 07 |
| Little's Law | 07 | L = λW, near-definitional, true of any stable queue; two knowns give the third | cite 07 |
| Utilization is superlinear | 07 | Wait scales as 1/(1−ρ); there is no cliff at 85%, only a marginal cost that rises from the start | cite 07 |
| Memory hierarchy ~6 orders | 07 | Register to network spans about a million-fold | cite 07 |
| The cache line is the transfer unit | 07 | Cost is set by how much of each fetched line you use, which is layout not algorithm | cite 07 |
| Speed of light as a floor | 07 | Cross-region round trips have a floor no profiling removes; change geography or stop waiting | cite 07 |
| The crossover is not a property of the algorithms | 07 | Where O(n) loses to O(1) is set by comparison cost against hash cost — measure yours | cite 07 |
...
| Conway / Brooks / Lehman | 08 | Structure mirrors org; adding people to a late project; systems must change | cite 08 |
| Lehman's ratchet | 08 | Complexity rises because adding a case is cheap and local while removing one needs a project | cite 08 |
...
| A rename is a silent break | 08 | Removals and renames fail without an error; a type change at least fails loudly | cite 08 |
| You cannot deploy other people's software | 08 | The one constraint in the book that cannot be fixed by changing code you control | cite 08 |
| Inverse Conway is a strategy, not a finding | 08 | The observation is established; driving architecture by reshaping teams is not | cite 08 |
| Conway detects mismatch | 08 | Neither tight coupling nor a firm interface is better in itself; the failure is seams landing where the work was divided rather than where the problem divides | cite 08 |
| Conway as a homomorphism | 08 | Every subsystem maps to one design group and every interface to a negotiation; the direction gives many-services-per-team but never many-teams-per-service | cite 08 |
...
| One team per service, read correctly | 08 | A constraint on who may own a service, not a recipe for how many to have | cite 08 |
...
| Long feedback loops | 08 | These decisions cannot be tested, because the evidence arrives after the cost is sunk | cite 08 |
| Compression + constraint tests | 09 | A pattern earns its name by saving words and ruling something out | "the two tests (Ch. 09)" |
| A pattern is not one of the five kinds | 09 | The kinds classify claims; a pattern is a name for a shape, and names are not true or false | cite 09 |
| The tests are independent | 09 | Facade compresses and constrains nothing — four outcomes, not a single axis | cite 09 |
| Compression needs a shared referent | 09 | The saving exists only for a reader who knows the term; a local coinage compresses nothing outside | cite 09 |
| The constraint test is mechanical | 09 | Try to write code the name forbids; if you can and still use the name, it constrains nothing | cite 09 |
| A catalogue is ethnography | 09 | It records shapes that were occurring; read as a checklist it becomes obligations. 03 owns describes-vs-prescribes | cite 09 |
| A name as a search term | 09 | Some names are mediocre descriptions and good indexes into the literature on their failure modes; this is also all a weak name gives a learner | cite 09 |
| Too young to name | 09 | The tests assume you can say what the code does; before that a vague name is honest, and the failure is losing track that it is provisional | cite 09 |
| Directories group by change, names group by shape | 09 | A folder named for a pattern collects code belonging to different features | cite 09 |
...
| The third option | 10 | At class scale you can change the other side instead of adapting; at system scale that option is gone | cite 10 |
| Shapes are scale-free, Forces are not | 10 | A pattern name transfers the picture and drops the constraints, which were the expensive half | cite 10 |
| Crossing the line adds failure modes | 10 | The same shape acquires loss, duplication, latency, and published commitment — Part II's Laws, one at a time | cite 10 |
| Unchanged by the boundary | 10 | If you cannot state what the pattern would be on the other side, it is a code-organization device — Strategy, Template Method | cite 10 |
| Singleton inverts | 10 | "Exactly one" across machines is leader election, which needs consensus (Ch. 06) | cite 10 |
...
| Patterns sorted by Force | 11 | Chapter 02's seven Forces, in chapter 02's order and names; 49 patterns sort, 5 refuse | cite 11 |
| Team size relocates rules | 11 | This Force changes where a rule lives rather than what it is, so it produces few patterns of its own | cite 11 |
...
| Same Force, several answers | 11 | The Force narrows the field; the intensity picks the answer, and 19 does the choosing | cite 11 |
...
| Norvig's count, with its qualifier | 12 | 16 of 23 "invisible **or simpler**", "for at least some uses" — and three levels, invisible/informal/formal, not two | cite 12; 03 owns dropped quantifiers |
...
| Visitor expired in place | 12 | Java grew sealed types and pattern matching, so the pattern died in the language that entrenched it — not by translation elsewhere | cite 12 |
| The dissolving four are one request | 12 | Command, Strategy, Template Method and Visitor all ask the caller to supply behaviour; one feature answers all four | cite 12 |
| Composite outlives the feature that killed Visitor | 12 | Sum types dissolve one and not the other in the same file, because containment is a claim about the domain (Ch. 11) | cite 12 |
| The translation test is scoped | 12 | Observer dissolves in one process and returns across a machine; run at the wrong scope the test returns a confident wrong answer | cite 12 |
| The test names the target language | 12 | "Visitor is a workaround" is a claim about a pair, and is true and useless if your compiler lacks the feature | cite 12 |
...
| Interface width, not language, limits decoration | 12 | Five methods means four forwarding methods no feature removes, because they are not simulating anything (Ch. 04) | cite 12 |
| The scaffold's failure modes move to the feature | 12 | Not *you lose the literature* — decorator gotchas become function-composition gotchas, and order is the big one | cite 12 |
...
| Neither shape nor verdict | 13 | *Interesting approach* names nothing and asserts nothing, which is its function; a word from that cell means no design feedback was given | cite 13 |
| Shape-plus-verdict is the damaging cell | 13 | The shape is checkable so the term reads as description, and the verdict rides along unexamined; this is the pattern-vocabulary case | cite 13 |
...
| Marking a hunch makes it usable | 13 | Stated as the speaker's state and converted into a question about why this shape, a hunch becomes the Forces question; unmarked it is a verdict with no subject | cite 13 |
| Two axes give two orderings | 13 | By *can you dissent* the shape-plus-verdict cell is worse; by *is there anything to check* the no-shape cell is worse, and neither is the ordering | cite 13 |
| Anemic drops an antecedent | 13 | Fowler's argument is that you incurred a domain model's costs and got none of its benefits; without the mapping layer and object graph there is no wasted payment to complain about | cite 13 |
| Behaviour is placed, not absent | 13 | What the rule must see — how much data you must be looking at before you can tell whether it holds — decides where a rule can live; 05 owns why the widest case is not a preference | "placed by what the rule must see (Ch. 13)" |
| Verdict nouns are legitimate for Laws | 13 | SQL injection carries its verdict correctly because the condition attached is *always*; *premature optimization* does not, because whether it is premature is a latency-budget question | cite 13 |
| A term's cell is not fixed | 13 | *Monolith* crossed the verdict axis and back while never leaving the top row — the tests measure a term in a community at a time | cite 13 |
| Compressing well is why a verdict noun spreads | 13 | It passes 09's compression test easily, which is what carries it; 14 owns why the qualifier is the part that goes | cite 13 |
...
| A proverb is a pointer, not a container | 14 | Segoe's form is opaque by design — Pike reads two out and says don't worry whether you understand them; the phrase indexes the teaching rather than replacing it | cite 14 |
| Board proverbs are predictive | 14 | They say what will happen and decline to say whether you want it; several of Pike's took imperative mood instead, and the change was unremarked | cite 14 |
...
| Designed to be spoken, not published | 14 | *I think you know them already… ideas you might use to explain to somebody* — the speaker carries the scope, the sentence is the handle, and Pike predicted the page that would carry the handle alone | cite 14 |
| The reader resolves outward | 14 | *Sharing memory* has no fixed extent, so a reader resolved it to any memory two goroutines can reach; self-reported, and the code that prompted it is race-clean | cite 14 |
| Scope gets rebuilt by hand | 14 | `MutexOrChannel`'s table reconstructs Pike's gloss; commenters reconstruct it again; a meta-proverb gets improvised — the repair Sensei's Library institutionalised as a category | cite 14 |
...
| The test measures checkability, not fit | 14 | Passing it means the principle handed you something to compare your case against, not that it handed you the correct extent | cite 14 |
...
| Unconditional advice loses nothing | 14 | gofmt travelled intact because there is no situation in which one consistent format is wrong; 14's boundary in a different costume | cite 14 |
...
| Locally applicable, globally evaluable | 15 | The instruction can be followed one method at a time; the cohesion test measures every method at once and the cycle needs two rules resolved to opposite sides — so what compresses is exactly the part that survives being applied alone | cite 15 |
| Mocks assert about mocks | 16 | A mocked test passes when the real constraint has been deleted | cite 16 |
...
| The bundling is in the teaching, not the canon | 16 | Beck's own statement of the loop mentions no mocking, no isolation and no speed requirement; the fast-tests-therefore-doubles chain is attached to TDD elsewhere | cite 16 |
| A double encodes only known constraints | 16 | A fake is written by the same author as the real rule and agrees with it by construction, so it cannot disagree with a schema that is wrong | cite 16 |
...
| Ask it at release scope too | 16 | *If this broke in production tomorrow, could we say the cause is not ours because these tests would have caught it* — reaches the dependencies faked below production and the fixture data tidier than anything real | cite 16 |
...
| Test-first couples tests to structure | 16 | The test names an interface before it has settled, so it encodes shape as well as behaviour, and structural change costs test change in proportion | cite 16 |
| Layered packages force exports | 20 | A directory wall requires publishing the helpers it was meant to hide — verified: `undefined: store.scanOrder`, then `go doc` listing the exported helper | cite 20 |
...
| The caveat was printed and still lost | 17 | Martin's last paragraph says the standard may suit only certain applications and that he would regret unconditional conformance — 15's mechanism on advice whose scope was written down, not merely spoken | cite 17 |
...
| Injection is not abstraction | 17 | Passing a dependency in and hiding it behind an interface are two decisions; 05 argues for the first, and only the second is the speculative one | cite 17 |
...
| The lowest common denominator is unknown | 17 | It is the intersection of feature sets for engines nobody has chosen, so it gets approximated by superstition — `for update` is absent from SQLite, `on conflict` is not | cite 17 |
...
| *Later* is the tell | 17 | An interface whose reason survives deleting the word *later* is one chapter 04 would defend | cite 17 |
| Force-map method | 18 | Read forces, derive principles, check idioms — in that order | cite 18 |
| FlowCore decision 12 as a worked map | 18 | Deep Get in a repeatable-read transaction: the transaction forced by concurrency and blast radius, four-queries-over-a-join chosen, completion-path locking deferred, and the trigger that would reopen it | cite 18 |
...
| What a force map adds to an ADR | 18 | Nygard's Context already asks for *the forces at play*; the map adds forced-against-chosen, which says what is safe to touch, and *revisit if*, which is a trigger written before rather than a Status marked after | "what the map adds (Ch. 18)"; 11 owns the ADR pattern |
...
| The method needs the expertise it looks like it replaces | 18 | Deriving from a force requires already knowing what the force implies and what the options cost; a map filled in by someone who cannot price them is confident and wrong | cite 18 |
| Inherited principles cluster | 18 | A codebase carries the whole set that travelled together, so finding one unforced principle is a reason to look at its neighbours | cite 18 |
...
| The Idiom/Style line moves by language | 20 | Go makes an identifier's case an access modifier and Python makes indentation syntax, so the two things everyone files under Style are structural there. 01 owns the mechanical test; 20 owns the fact that it lands in a different place per language | "where the line falls (Ch. 20)" |
| One decision, three ecosystems | 20 | Go owns `main` so a container buys nothing; C#'s framework constructs the controllers so it must resolve them; Python has both conditions in different projects and adopts injection where a per-request lifetime is real. 01 owns the demonstration, 21 the explanation | cite 20 |
...
| A deviation, declared and dated | 20 | The early Go compiler in C: reason stated, offence taken, and the reason later expired and the deviation reversed | cite 20 |
...
| Profile inversions | 19 | The forces a profile pins invert some standard advice | cite 19 |
...
| A Style argument deposits nothing | 21 | A Force argument that runs six days can end in a measurement everyone now has; a Style argument cannot end that way, so "produces nothing" is literal rather than a complaint about waste | cite 21 |
...
| Enforcing late costs more than early | 21 | A formatter run over an established codebase rewrites files nobody edited, so `git blame` on those lines names whoever ran it — the same decision is cheap on day one and expensive on day one thousand | cite 21 |
| A profile disagreement does not resolve | 19 | 02 owns the mechanism; 19 owns only the stability. An ordinary force disagreement ends when somebody measures, and two people reading different profiles are each reading a force that will not move in their own system | "stably different (Ch. 19)" |
| A force outside its ordinary range overturns a family | 19 | Not one piece of advice but every piece that depended on the same thing — which is the signature that distinguishes a profile from a special case | cite 19 |
...
| Corpus monoculture | 01 | A model has one training distribution and cannot acquire a second, so 01's prescribed cure is unavailable to it | cite 01; argument in `docs/pending-tasks/ai-material.md` |
| An AI coding agent cannot see your Forces | 02 | Forces are facts about your situation; an agent has the prompt, so the groundwork is skipped by construction | cite 02; argument in `docs/pending-tasks/ai-material.md` |
...
| The interview does not improve the answer | 18 | It makes the answer disagreeable-with. The same two decisions exist in generated code as a column default and a v4 constructor, taken by whatever is most common and unmarked; 22 owns that case | "answers somebody can disagree with (Ch. 18)" |
| Fact and decision have different owners | 22 | Grilling's load-bearing line: facts get looked up, decisions get put to the human — steps one and two of the method, separated and given owners, which is what makes the output auditable | "fact or decision (Ch. 22)" |
| Grilling | 22 | An interview that surfaces each decision, with a recommendation, before anything is written; the human supplies the Force that settles it | "grilling (Ch. 22)"; text, provenance and limits in `docs/pending-tasks/ai-material.md` |
| Grilling's first limit | 22 | It surfaces decisions the agent recognizes as decisions, so it is weakest where the corpus is most uniform | cite 22; argument in `docs/pending-tasks/ai-material.md` |
...
| Behaviour is re-derivable, reasons are not | 22 | Asking what the code does is reading and works at any time; asking why this shape was chosen is not, because it was never in the artifact and no amount of freshness puts it there | cite 22 |
...
| The record is the only copy | 22 | For a human author memory covers the gap while it fades; remove the memory and there is no interval in which the reason is available and undocumented, so the record is not a backup of anything | cite 22 |
...
| `split(8.03, 3)` | 02 | Blast radius: literally the same function, correct behind a dashboard and defective on an invoice |
| Payment method: switch, registry, six layers | 02 | Change frequency plus shape; the six-file version stubbed out so the count is checkable. 17 owns what the boundaries cost |
| `Money` as a comment vs unexported fields | 02 | Team size: the same rule migrates from comment to type system. 11 owns the technique |
...
| `LoggingOrderService` vs middleware | 04 | A cross-cutting concern made into a layer; per-method forwarding tax. 12 owns Decorator-as-composition |
| Mutually recursive `parseExpr`/`parseTerm` | 04 | A cycle that costs nothing, because the two were never separate units |
| `Particle` class vs ECS parallel arrays | 04 | Hiding inverted by cache layout — the Principle turns over while the Law holds. 19 may cite for the domain, 07 owns the arithmetic |
| `net/http` `Handler` | 04 | Call up, dependency down — the legitimate version of an apparent violation |
| `completed_at is null` gate | 04 | The lower layer is more capable, so layering doctrine inverts. 05 owns the race it closes |
...
| In-process channel vs lost ACK | 03 | Two Generals' two assumptions, and the two different escapes: in-process falsifies one, idempotency drops the requirement. 06 owns the theorem |
...
| Go map randomization vs Python dict order | 03 | One regularity, two opposite responses, and a magnitude that moved. 04 owns Hyrum's Law |
| Summing one field across 2M order records vs one column | 07 | 7.1x from where the bytes sit; 120-byte record, 64-byte cache line, no algorithm change. 04 owns the encapsulation argument and keeps `Particle` |
...
| Code each name forbids | 09 | Singleton and Transaction Script forbid something; Facade forbids nothing. 10 owns Facade at scale |
...
| Batching measured: 1145 ms to 11 ms | 11 | Latency's arithmetic; 07 owns the curve |
