...
| `getBalance` / `setBalance` | 7nkn | The JavaBeans condition is real and about the toolchain — reading a property needs a getter, writing one needs a setter, and most fields are never written that way. That the two arrive as a pair is the half nothing asked for, and it costs the type every invariant it might have held. Records and `init`-only properties are the shape the condition called for |
