...
| Force-map method | r37x | Read the Forces, take the Laws they make binding, derive the Principles they support, check the Idioms last — in that order | cite |
| Binding Laws bound the space first | r37x | A Force decides whether a Law has anything to act on, so the reading settles which Laws bind before any Principle is chosen; cjx4 owns the asymmetry, this is the step that uses it | cite |
| The artifact does not record its order | r37x | An interface derived from a Force and one added because every repository here has one are the same file, so the order has to be held rather than checked after the fact — 4jk6 and u8eu are the chapter-length cases | "the result is where the difference is not" |
