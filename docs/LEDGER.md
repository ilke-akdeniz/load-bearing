...
| Why the kinds get confused | cjx4 | Tone doesn't vary with authority; advocacy compresses; monoculture makes Idioms feel universal. Pike is the witness on the first, about his own language; and one generator returns all five kinds in one register, so there is nobody whose confidence can be discounted | one clause, cite |
...
| *Force* is a borrowed word | cjx4 | A named element in the literature on writing patterns, from Alexander — not in the Gang of Four template, which is what most readers mean by patterns. There it is a field describing the tensions one pattern resolves; here it is a property of your situation read before any pattern is in view, and a kind of claim rather than a template slot | "the word is the pattern-writing literature's" |
...
| Volume removes the cheap alternative to mutation | u8eu | Spot-checking rests on the author's list of assertions they were unsure about; tests generated in bulk arrive without one, and the ones worth mutating are the ones nobody formed an opinion about | cite |
...
| Corpus monoculture | cjx4 | A model has one training distribution and cannot acquire a second, so 01's prescribed cure is unavailable to it — a claim about what *idiomatic* can mean with one corpus, not about output quality | cite |
| The team-size Force at its extreme | f4m5 | A contributor present for no conversation, keeping nothing between sessions, at a volume review was not sized for. The comment → review habit → type system migration is forced harder and sooner, skipping the middle, because what a comment relies on is somebody remembering. Forces reach it only as far as a prompt carried them, since they are facts about your situation | "forced harder and sooner" |
| The scope was never set | b86v | For generated design nobody ever fixed an extent, so there is no author to ask and no talk to re-watch | "no scope was set"; argument in `docs/pending-tasks/ai-material.md` |
...
| Grilling | at4r | An interview that surfaces each decision, with a recommendation, before anything is written; the human supplies the Force that settles it | "grilling"; text, provenance and limits in `docs/pending-tasks/ai-material.md` |
| Grilling's first limit | at4r | It surfaces decisions the agent recognizes as decisions, so it is weakest where the corpus is most uniform | cite; argument in `docs/pending-tasks/ai-material.md` |
...
| Silent defaults | at4r | Generated code states no decisions at all — a taken branch leaves no mark, so review cannot catch what it never suspected | "silent default"; argument in `docs/pending-tasks/ai-material.md` |
...
| Fucci et al.'s four dimensions | u8eu | GRA, UNI, SEQ, REF and which survived model selection |
