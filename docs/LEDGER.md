...
| `getBalance` / `setBalance` | 7nkn | The JavaBeans condition is real and about the toolchain — serializers and ORMs find fields by accessor pair. The setter does not follow from it: anything a caller could do to a public field it can do through the setter, so the type holds no invariant |
