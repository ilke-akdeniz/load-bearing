...
| Interfaces added by reflex | agjy | An interface at every boundary whether or not the boundary was real, and the legitimate uses that make one implementation fine — narrowing what a consumer reaches, breaking a cycle, declaring a consumer-owned seam. 4jk6 says in its own text that agjy owns these; u8eu owns where the reflex comes from | "by reflex" |
