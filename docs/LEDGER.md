...
| The Idiom/Style line is mechanical | 02 | The compiler or the runtime acts on an Idiom while the program behaves the same either way; neither can see a Style. Ecosystem-specificity does not discriminate, because plenty of Style is local. 21 owns the fact that the line falls in a different place in each language | "the machine can see it (Ch. 02)" |
