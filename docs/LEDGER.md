...
| An unreadable Force forces a Law | cjx4 | Where a Principle's conditioning Force has no instrument — *is somebody trying* — the classification is correct and useless, so the conditionality is dropped and the advice followed absolutely. f4m5 owns risk-versus-Force; this is what it costs at the point of classifying | cite |
| One implementation, no plausible second | cjx4 | Listed as a recognition symptom only; 4jk6 owns the argument, r8dw owns the version that wraps a type you own, and esqm owns the one-method field | "and no plausible second one" |
