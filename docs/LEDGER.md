...
| One principle generates the other | 17 | The loop runs at minutes, so the suite must answer in seconds, so the database leaves the test — the granularity carrying the measured benefit is the same granularity that produces the pressure to mock. This book's connection, not the paper's | "the loop forces the mock (Ch. 17)" |
...
| A ritual supplies cadence without belief | 17 | The loop forces small even steps whether or not the follower accepts the reason; dropping it means producing the cadence deliberately, with nothing to signal when you stop | cite 17 |
