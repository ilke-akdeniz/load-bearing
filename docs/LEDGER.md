...
| `getBalance` / `setBalance` | 7nkn | The JavaBeans condition is real and about the toolchain, but the setter came from how a deserializer builds an object — no-arg constructor, then assign — so it was about construction and covered only deserialized fields. The convention made writability permanent and universal, costing the type every invariant. Records and `init` let the tooling write once at construction and never after |
