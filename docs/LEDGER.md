...
| An AI coding agent cannot see your Forces | 03 | Forces are facts about your situation; an agent has the prompt, so the groundwork is skipped by construction | cite 03; argument in `docs/pending-tasks/ai-material.md` |
...
| Grilling's first limit | 23 | It surfaces decisions the agent recognizes as decisions, so it is weakest where the corpus is most uniform | cite 19; argument in `docs/pending-tasks/ai-material.md` |
| What persists is text | 23 | A forward pass discards its activations and every persistence mechanism these tools have stores tokens, so there is never a replay — which is why the chapter rests on that rather than on the contested question of whether stated reasoning reflects computation | "what persists is text (Ch. 23)" |
| Three cases, and the middle one does the damage | 23 | Same session with the reasoning written out is retrieval of what was said; same session with nothing written is a fresh computation on overlapping input, correlated and not a recollection; a new session has only the artifact. All three read alike from outside | "the middle case (Ch. 23)" |
| Behaviour is re-derivable, reasons are not | 23 | Asking what the code does is reading and works at any time; asking why this shape was chosen is not, because it was never in the artifact and no amount of freshness puts it there | cite 23 |
| Folk remedy | 23 | Advice applied far outside the context it was made for, which stays misapplied because nobody rebuilds its scope — the author's term, not standard. *Depend on abstractions* is the worked case (Ch. 18); a corpus default is the purest instance, since there nobody knows a scope existed | "a folk remedy (Ch. 23)"; 15 hands it over |
| Grilling's second limit: granularity | 23 | The interview only reaches decisions at the scale of the request, so asking for a whole application never separates out the trade-offs that live inside its fourth piece. FlowCore's slices, with scope written into standing instructions — *do not build ahead into these* — are the countermeasure | "the granularity limit (Ch. 23)" |
| An entry is reusable to the extent it records why | 23 | *Full-word identifiers everywhere* transfers nothing; *abbreviations must be decoded rather than read* can be checked against new readers and kept or dropped. A conclusion does not travel, a conclusion with its condition does — 15's mechanism running forwards | "why, not what (Ch. 23)" |
| Grilling leaves two artifacts | 23 | The log, and a person who now holds the trade-off. Accepting every recommendation is still not rubber-stamping if you understood what was being chosen between — and an agent keeps nothing between sessions, so the connection to next month's question has to live in a person or a document | "the second artifact (Ch. 23)" |
| The record is the only copy | 23 | For a human author memory covers the gap while it fades; remove the memory and there is no interval in which the reason is available and undocumented, so the record is not a backup of anything | cite 23 |
| Unrecorded decisions compound | 23 | Each one constrains the next change without saying so, and removing anything requires knowing what it was for — which ends in requests that can only be negative, *fix this, do not break that* | "fix this, don't break that (Ch. 23)" |
| Self-enforcing beats recorded | 23 | Where a decision can be written into something that refuses to be violated, that is worth more than a record, because enforcement does not depend on anyone reading anything. Narrow: most design decisions are judgements, not invariants | "make it self-enforcing (Ch. 23)" |
...
| `get_definition` with and without its transaction | 23 | Both run; with a concurrent edit between the two reads the second returns revision 1 of a definition with three steps, which was never saved. 19 maps the same FlowCore decision as a log entry; this shows it in the code, where none of the reasoning is visible |
| `UNIQUE (definition_id, active)` | 23 | The boundary: the decision is in the schema, so violating it raises `IntegrityError` at the moment of the change and nobody needs to remember why |
