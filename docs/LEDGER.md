...
| Priority churn | i59b | How often work in flight is displaced by a decision taken outside the team, and what fraction reaches closure. What it decides is the size of the unit of work — enough must finish between displacements — rather than how far ahead a plan reaches | "does it reach closure" |
| *Changing requirements* usually names the wrong thing | i59b | Business invariants are facts about how a company makes money and hold for years; what moves is which of them you were asked to serve. Rules never gathered properly surface later as a change, when nothing changed except who read them — both reach the team as churn and only one is anybody's mind altering | "which of the two churns" |
...
| X, then Y, then X again | i59b | Priority churn: six weeks, nothing shipped, no estimate missed and nobody slow — the switches were decided by people entitled to decide them, without reading anything |
