...
**Owed to:** chapter 20, *Idioms: why ecosystems diverge*. One section, not a chapter.

**Origin.** This was chapter 18, *Clean Architecture versus the language*, drafted and then cut. Decision 83 records why: the chapter ran on *the database is a detail*, and the demonstration below does not follow from that advice. The export bill is a property of Go's package semantics and is paid identically whatever the motive for the split — which the chapter's own boundary section conceded.
